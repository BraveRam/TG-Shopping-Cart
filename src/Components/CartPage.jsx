  ...
  if(cart.totalQuantity > 0){
    return (
      ...
      <Modal isOpen={cart.modalOpen} title={"Purchase🎉"} isFlex={false} isSmall={false}>
        <h1 className="text-center font-light text-white">You've successfully purchased your products:)</h1>
      ...
      <button className="block bg-black border-[1px] border-white py-[7px] px-[20px] rounded outline-0 text-white font-bold fixed bottom-5 left-1/2 transform -translate-x-1/2 z-[100]" onClick={()=>dispatch({type: "OPEN_MODAL"})}>Pay Checkouts<br></br>$ {cart.totalPrice.toFixed(0)}</button>
      </div>
      )
  }
  return (
     <div className="w-[90%] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7 mx-auto mt-20 shadow-md mb-3">
        <h1 className="text-3xl text-white text-center mt-30 font-extrabold mx-auto">Empty Cart</h1>
     </div>
    )
}